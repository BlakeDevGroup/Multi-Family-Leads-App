...
describe("PropertyJsonQuery", () => {
  ...
  describe("readById", () => {
    it("should send success when id is found", async () => {
      ...
      expect(spySuccess.args[0][2]).to.equal(200);
    });
    ...
    it("should catch error and send failure", () => {
      let stub = sandBox.stub(Array.prototype, "filter").throws(ERROR);
  ...
  describe("list", () => {
    ...
    it("should catch error and send failure", () => {
      let stub = sandBox.stub(Array.prototype, "map").throws(ERROR);
